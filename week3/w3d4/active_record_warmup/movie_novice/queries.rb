...
def top_titles
  ...
  Movie.select('movies.id, movies.title').where('score >= ?', 9)
end

def star_wars
  #display the id, title and year of each Star Wars movie in movies.
  # hint: use 'select' and 'where'
  Movie.select('movies.id, movies.title, movies.yr').where('title LIKE ?', '%Star Wars%')
end
...
def uma_movies
  ...
  Movie.select('movies.id, movies.title, movies.yr').joins(:actors).where('actors.name = ?', 'Uma Thurman')
end
